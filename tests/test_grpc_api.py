...
def _mock_pipeline_result(hash_id, country_matched, country_conf, country_resolved,
                          ...
                          country_start=0, country_end=5, town_start=10, town_end=15):
...
def _mock_empty_pipeline_result(hash_id):
...
def mock_pipeline():
    mock_pipe = Mock()
    mock_pipe._batch_size = 1024
    return mock_pipe


@pytest_asyncio.fixture
async def grpc_channel(mock_pipeline):
    """Start a gRPC server with the mocked pipeline and yield a channel to it."""
    server = grpc.aio.server()
    pb2_grpc.add_AddressStructuringServicer_to_server(
        AddressStructuringServicer(mock_pipeline), server
    )
...
def stub(grpc_channel):
    return pb2_grpc.AddressStructuringStub(grpc_channel)


def _sample(text, hash_id, suggested_country=None, force_suggested_country=False):
...
class TestProcessAddress:
    ...
    @pytest.mark.asyncio
    async def test_single_address(self, stub, mock_pipeline):
    ...
    @pytest.mark.asyncio
    async def test_multiple_addresses(self, stub, mock_pipeline):
    ...
    @pytest.mark.asyncio
    async def test_null_matches_skipped(self, stub, mock_pipeline):
        """When both country and town are None for a given rank, that match is skipped."""
        mock_pipeline.run.return_value = [
            _mock_empty_pipeline_result("idx1")
        ]
    ...
    @pytest.mark.asyncio
    async def test_empty_stream(self, stub, mock_pipeline):
    ...
    @pytest.mark.asyncio
    async def test_suggested_country_forwarded(self, stub, mock_pipeline):
        """Verify suggestedCountry and forceSuggestedCountry reach the pipeline reader."""
        mock_pipeline.run.return_value = []
